    ...
    <div class="mx-auto w-full max-w-6xl px-6">
        ...
        <div class="mt-10 grid gap-x-8 gap-y-14 sm:mt-12 lg:grid-cols-2">
            ...
            <a href="{{ $item->link }}" class="group block" data-reveal>
                ...
                <p class="mt-1.5 max-w-[52ch] text-base/7 text-muted sm:text-sm/6">{{ $item->summary }}</p>
                <p class="mt-3 font-mono text-[0.6875rem] tracking-wide text-faint">{{ $item->role }} · {{ $item->stack }}</p>
            </a>
            @endif
            @endforeach
        </div>
