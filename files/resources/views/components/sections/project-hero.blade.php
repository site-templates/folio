...
<section class="relative pt-40 pb-4 sm:pt-44">
    <div class="mx-auto w-full max-w-6xl px-6">
        <p class="crosshair inline-block font-mono text-[11px] tracking-widest text-faint uppercase" data-reveal>{{ $label }} · {{ $project->year }}</p>

        <h1 class="mt-8 max-w-[18ch] text-[2.6rem] leading-[1.06] font-display font-medium tracking-[-0.025em] text-balance sm:text-[4rem]" data-reveal>{{ $project->title }}</h1>

        <div class="reveal-1 mt-8 flex flex-wrap items-end justify-between gap-x-16 gap-y-8" data-reveal>
            <p class="max-w-[50ch] text-lg/7 text-pretty text-muted">{{ $project->summary }}</p>

            <dl class="grid shrink-0 grid-cols-2 gap-x-10 gap-y-4 text-sm sm:grid-cols-4">
                <div>
                    <dt class="font-mono text-[11px] tracking-widest text-faint uppercase">{{ $labelClient }}</dt>
                    <dd class="mt-1 text-ink">{{ $project->client }}</dd>
                </div>
                <div>
                    <dt class="font-mono text-[11px] tracking-widest text-faint uppercase">{{ $labelYear }}</dt>
                    <dd class="mt-1 text-ink">{{ $project->year }}</dd>
                </div>
                <div>
                    <dt class="font-mono text-[11px] tracking-widest text-faint uppercase">{{ $labelRole }}</dt>
                    <dd class="mt-1 text-ink">{{ $project->role }}</dd>
                </div>
                <div>
                    <dt class="font-mono text-[11px] tracking-widest text-faint uppercase">{{ $labelStack }}</dt>
        ...
        <p class="reveal-2 mt-6" data-reveal>
            <a href="{{ $project->external }}" rel="noopener" class="group inline-flex items-center gap-2 text-sm font-medium text-ink">
                ...
                <svg viewBox="0 0 16 16" class="size-3.5 fill-current transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden="true">
