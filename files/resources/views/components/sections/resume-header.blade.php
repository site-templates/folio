...
<section class="relative pt-40 pb-4 sm:pt-44">
    <div class="mx-auto w-full max-w-6xl px-6">
        ...
        <div class="mt-8 flex flex-wrap items-end justify-between gap-x-12 gap-y-8">
            <div>
                <h1 class="text-4xl font-display font-medium tracking-tight sm:text-6xl" data-reveal>{{ $name }}</h1>
                <p class="reveal-1 mt-2 text-xl text-muted" data-reveal>{{ $role }}</p>
                <p class="reveal-2 mt-6 max-w-[56ch] text-[0.9375rem]/7 text-pretty text-muted" data-reveal>{{ $summary }}</p>
            ...
            <div class="reveal-3 flex flex-col items-start gap-3" data-reveal data-print-hide>
                <a href="{{ $downloadLink }}" class="inline-flex items-center gap-2.5 rounded-full bg-accent py-2.5 pr-2.5 pl-5 text-sm font-medium text-accent-ink hover:opacity-85">
                    {{ $downloadText }}
                ...
                <p class="font-mono text-[0.6875rem] text-faint">{{ $printHint }} <kbd class="key">⌘</kbd> <kbd class="key">P</kbd></p>
            </div>
            @endif
        </div>
    </div>
