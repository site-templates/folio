...
<footer class="relative border-t border-line">
    <div class="mx-auto w-full max-w-6xl px-6 py-14 sm:py-16">
        <div class="flex flex-wrap items-start justify-between gap-x-12 gap-y-10">
            ...
            <div>
                <a href="/" aria-label="Homepage" class="inline-flex items-center gap-2.5 text-ink">
                    ...
                    <span class="text-[0.9375rem] font-semibold tracking-tight">{{ $name }}</span>
                </a>
                <p class="mt-4 max-w-[30ch] text-base/7 text-pretty text-muted sm:text-sm/6">{{ $tagline }}</p>
            </div>
        ...
        <div class="mt-12 flex flex-wrap items-center justify-between gap-x-8 gap-y-4 border-t border-line pt-6">
            ...
            <p class="font-mono text-[0.6875rem] tracking-wide text-faint">{{ $metaLine }}</p>
            @if ($showTop)
            <a href="#" class="group inline-flex items-center gap-1.5 text-sm text-muted hover:text-ink">
                Back to top
                <svg viewBox="0 0 16 16" class="size-4 fill-current transition-transform duration-200 group-hover:-translate-y-0.5" aria-hidden="true">
